...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def _register_command_menu(self) -> None:
        ...
        if expose_globally:
            ...
            if hidden_count:
                logger.info(
                    "[%s] Telegram menu: %d commands registered globally, %d hidden (over %d limit). Use /commands for full list.",
    ...
    async def _run_post_connect_housekeeping(self) -> None:
        ...
        try:
            # Register bot commands only for authorized chats unless the
            # operator explicitly opts into global menu exposure.
            try:
