...
@pytest.mark.asyncio
async def test_telegram_model_picker_can_be_hidden_by_profile_config(tmp_path, monkeypatch):
    hermes_home = tmp_path / ".hermes"
    hermes_home.mkdir()
    (hermes_home / "config.yaml").write_text(
        yaml.safe_dump(
            {
                ...
                "telegram": {"show_model_choices": False},
            }
        ),
        encoding="utf-8",
    )
    ...
    assert "Current model: `gpt-5.4`" in result
    assert "Model choices are hidden for this Telegram profile" in result
